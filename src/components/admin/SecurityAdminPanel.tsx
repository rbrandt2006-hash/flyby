...
type TabId = "audit" | "sessions" | "policies";
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="py-8 text-center text-muted-foreground text-sm">
                ...
                No audit events recorded yet. Events will appear as users interact with the platform.
              </div>
            ) : (
              <div className="space-y-1 max-h-[500px] overflow-y-auto">
                {auditLogs.map((log) => (
                  <div
                    key={log.id}
                    className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-secondary/30 transition-colors text-sm"
                  >
                    <Badge
                      variant="secondary"
                      className={cn("text-[10px] font-medium shrink-0", actionColors[log.action] ?? "bg-muted")}
                    >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium">
                          ...
                          {i === 0 && (
                            <Badge variant="secondary" className="ml-2 text-[10px] bg-success/10 text-success">
                              Current
                            </Badge>
                          )}
                        </p>
          ...
          <Card className="border-border/50">
            ...
            <CardContent className="space-y-3">
              ...
              <PolicyRow
                ...
                value="Uppercase, lowercase, number, special char"
                status="enforced"
              />
              <PolicyRow
                label="Session timeout (standard users)"
                value="30 minutes of inactivity"
                status="enforced"
              />
              <PolicyRow
                label="Session timeout (admin users)"
                value="15 minutes of inactivity"
                status="enforced"
              />
              <PolicyRow
                label="Login lockout threshold"
                value="5 failed attempts / 15 min"
                status="enforced"
              />
              <PolicyRow
                label="MFA for privileged roles"
                value="SMS-based 2FA available"
                status="available"
              />
              <PolicyRow
                label="Email confirmation required"
                value="Users must verify email before login"
                status="enforced"
              />
              <PolicyRow
          ...
          <Card className="border-border/50">
            <CardHeader className="pb-3">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-base">Data Protection Policies</CardTitle>
              </div>
            </CardHeader>
            <CardContent className="space-y-3">
              <PolicyRow
                label="File upload types"
                value="PDF, PNG, JPG, JPEG only"
                status="enforced"
              />
              <PolicyRow
                label="Max file size"
              ...
              <PolicyRow
                ...
                value="RLS enforced on all tables"
                status="enforced"
              />
              <PolicyRow
                label="Audit logging"
                value="Append-only, all critical actions"
                status="enforced"
              />
              <PolicyRow
                label="API rate limiting"
                value="Per-endpoint policies on edge functions"
                status="enforced"
              />
              <PolicyRow
                label="Re-authentication for admin actions"
                value="Password re-entry required"
                status="enforced"
              />
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Enable leaked password checking in your backend settings to prevent users from using
